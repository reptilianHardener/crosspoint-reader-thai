...
void EpdFont::getTextBounds(const char* string, const int startX, const int startY, int* minX, int* minY, int* maxX,
  ...
  *maxY = startY;
  ...
  while ((cp = utf8NextCodepoint(reinterpret_cast<const uint8_t**>(&string)))) {
    ...
    const int cursorXPixels = fp4::toPixel(lastBaseX);  // snap 12.4 fixed-point to nearest pixel
    const int glyphBaseX =
        isCombining ? (utf8IsThaiCombiningMark(cp) ? cursorXPixels : (combiningMark::centerOver(lastBaseX, lastBaseLeft, lastBaseWidth, glyph->left, glyph->width)))
                    : cursorXPixels;
