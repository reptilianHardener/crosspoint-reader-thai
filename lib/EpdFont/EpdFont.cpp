...
static inline bool fontHasThaiIntervalsBounds(const EpdFontData* fontData) {
...
static inline bool fontNeedsThaiUpperRestackingBounds(const EpdFontData* fontData) {
  if (!fontData) {
    return false;
  }
  return fontHasThaiIntervalsBounds(fontData);
}

static inline void applyThaiUpperStackingBounds(const EpdFontData* fontData, const uint32_t cp, const EpdGlyph* glyph,
                                                const int penY, int* raiseBy, int* stackedUpperMaxY,
                                                bool* hasStackedUpper) {
  if (!fontNeedsThaiUpperRestackingBounds(fontData)) {
    return;
  }
  ...
  while ((cp = utf8NextCodepoint(reinterpret_cast<const uint8_t**>(&string)))) {
    ...
    if (isCombining) {
      ...
      if (utf8IsThaiUpperLevelThreeMark(cp) && !hasStackedThaiUpper) {
        ...
        if (peekCp == 0x0E33) {
          const EpdGlyph* saraAmGlyph = getGlyph(0x0E33);
          if (saraAmGlyph) {
            stackedThaiUpperMaxY = startY + saraAmGlyph->top;
            hasStackedThaiUpper = true;
          }
        }
      }
      applyThaiUpperStackingBounds(data, cp, glyph, startY, &raiseBy, &stackedThaiUpperMaxY, &hasStackedThaiUpper);
    }
