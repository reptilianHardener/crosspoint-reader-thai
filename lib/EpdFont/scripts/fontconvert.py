...
intervals = [
    ...
    # Arrows
    (0x2190, 0x21FF),
    ### Miscellaneous Symbols (weather, zodiac, etc.; monochrome outlines) ###
    (0x2600, 0x26FF),
    ### Dingbats (hearts, hands, etc.) ###
    (0x2700, 0x27BF),
    ### CJK ###
...
for face in font_stack:
    # Bitmap-only faces reject set_char_size; prefer select_size when strikes exist.
    if face.is_scalable:
        face.set_char_size(size << 6, size << 6, 150, 150)
    elif len(face.available_sizes) > 0:
        face.select_size(0)
    else:
...
for i_start, i_end in intervals:
    for code_point in range(i_start, i_end + 1):
        ...
        pixel_mode = getattr(bitmap, "pixel_mode", freetype.FT_PIXEL_MODE_GRAY)

        if bitmap.width == 0 or bitmap.rows == 0:
            adv = fp4_from_ft16_16(face.glyph.linearHoriAdvance)
            glyph = GlyphProps(0, 0, adv, 0, 0, 0, 0, code_point)
            all_glyphs.append((glyph, b""))
            continue

        # Noto Color Emoji (and other COLR/CPAL fonts) expose BGRA bitmaps when FT_LOAD_COLOR is set.
        if pixel_mode == freetype.FT_PIXEL_MODE_BGRA and bitmap.width > 0 and bitmap.rows > 0:
            pitch = (bitmap.width // 2) + (bitmap.width % 2)
            buf = bytearray(bitmap.rows * pitch)
            for y in range(bitmap.rows):
                for x in range(bitmap.width):
                    off = (y * bitmap.width + x) * 4
                    b0 = bitmap.buffer[off]
                    g0 = bitmap.buffer[off + 1]
                    r0 = bitmap.buffer[off + 2]
                    a0 = bitmap.buffer[off + 3]
                    lum = (299 * r0 + 587 * g0 + 114 * b0) // 1000
                    lum = (lum * a0) // 255 if a0 else 0
                    nib = min(15, (lum * 15 + 127) // 255)
                    bi = y * pitch + (x // 2)
                    if x % 2 == 0:
                        buf[bi] = nib << 4
                    else:
