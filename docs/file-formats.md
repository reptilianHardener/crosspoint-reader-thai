...
The authoritative `section.bin` header version is `SECTION_FILE_VERSION` in [`lib/Epub/Epub/Section.cpp`](../lib/Epub/Epub/Section.cpp). Version 20 invalidates cached sections after Thai line-breaking / tokenization changes (layout may shift; regenerate `.crosspoint/.../sections/`).

### Version 8 (ImHex pattern reference; firmware may use a higher version)
